...
class AccountCheque(models.Model):
    ...
    def _get_in_count(self):
        for invoice in self.invoice_ids:
            if self.account_cheque_type == 'incoming':
                res =  self.invoice_ids.search([('state','=','open'),('partner_id','=',self.payee_user_id.name)])
                if not res:
        ...
        total = 0
        inv = self.invoice_ids.search([('reconciled','=',True),('partner_id','=',self.payee_user_id.name)])
        for order in inv:
